...
<style>
    ...
    .btn-danger-brutalist { background: #fee2e2; color: #dc2626; border: 2px solid #dc2626; padding: 16px 32px; font-size: 16px; font-weight: 800; text-transform: uppercase; cursor: pointer; box-shadow: 4px 4px 0 #dc2626; }
    .btn-danger-brutalist:hover { transform: translate(-2px, -2px); box-shadow: 6px 6px 0 #dc2626; }
</style>
@endpush
...
<div class="jf-wrap">
    <div class="flex justify-between items-center mb-8">
        <h1 class="jf-title">Edit Opportunity</h1>
        <form action="{{ route('hr.jobs.destroy', $job->id) }}" method="POST" onsubmit="return confirm('Archive this position permanently?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn-danger-brutalist">Close Posting</button>
        </form>
    </div>
    ...
    <form method="post" action="{{ route('hr.jobs.update', $job->id) }}">
        ...
        <div class="jf-section">
            ...
            <div class="jf-field">
                ...
                <input type="text" name="title" class="jf-input" required value="{{ old('title', $job->title) }}">
            </div>
            <div class="jf-field">
                <label class="jf-label">Short Summary</label>
                <textarea name="short_description" class="jf-textarea" rows="2" maxlength="255">{{ old('short_description', $job->short_description) }}</textarea>
            </div>
            <div class="jf-field">
                <label class="jf-label">Detailed Description <span class="req">*</span></label>
                <textarea name="description" class="jf-textarea" rows="10" required>{{ old('description', $job->description) }}</textarea>
            </div>
        </div>
        ...
        <div class="jf-section">
                ...
                <div class="jf-field">
                    ...
                    <select name="job_type" class="jf-select">
                        @foreach(\App\Enums\JobType::cases() as $type)
                            <option value="{{ $type->value }}" {{ old('job_type', $job->job_type->value) === $type->value ? 'selected' : '' }}>{{ ucfirst($type->value) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="jf-field">
                    ...
                    <select name="experience_level" class="jf-select">
                        @foreach(\App\Enums\ExperienceLevel::cases() as $level)
                            <option value="{{ $level->value }}" {{ old('experience_level', $job->experience_level->value) === $level->value ? 'selected' : '' }}>{{ $level->value }} Years</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="jf-field">
                ...
                <input type="text" name="skills" class="jf-input" value="{{ old('skills', is_array($job->skills_json) ? implode(', ', $job->skills_json) : '') }}" placeholder="React, Laravel, AWS...">
            </div>
            <div class="jf-field">
                <label class="jf-label">Benefits (Comma Separated)</label>
                <input type="text" name="benefits" class="jf-input" value="{{ old('benefits', is_array($job->benefits_json) ? implode(', ', $job->benefits_json) : '') }}" placeholder="Remote Work, Health Insurance...">
            </div>
        </div>
        ...
        <div class="jf-section">
            ...
            <div class="jf-row">
                <div class="jf-field">
                    ...
                    <input type="text" name="location" class="jf-input" value="{{ old('location', $job->location) }}">
                </div>
                <div class="jf-field">
                    <label class="jf-label">Deadline</label>
                    <input type="date" name="deadline" class="jf-input" value="{{ old('deadline', $job->deadline ? $job->deadline->format('Y-m-d') : '') }}">
                </div>
            </div>
            <div class="jf-row">
                <div class="jf-field">
                    <label class="jf-label">Min Salary (IDR)</label>
                    <input type="number" name="min_salary" class="jf-input" value="{{ old('min_salary', $job->min_salary) }}">
                </div>
                <div class="jf-field">
                    <label class="jf-label">Max Salary (IDR)</label>
                    <input type="number" name="max_salary" class="jf-input" value="{{ old('max_salary', $job->max_salary) }}">
                </div>
            </div>
        </div>

        <div class="flex gap-6 mt-12 pb-12">
            <button type="submit" class="jf-btn-submit">Update Posting</button>
